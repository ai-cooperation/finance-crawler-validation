...
為驗證「真的抓到內容」而不只驗證品牌級 metadata，`news_120` workflow 現在可用 `capture_raw=true` 將每個 endpoint attempt 的原始 response body 與 SHA-256 保存為 Actions artifact。加入非空 RSS item 檢查後，最新完整 [run 32119864710](https://github.com/ai-cooperation/finance-crawler-validation/actions/runs/32119864710) 實抓 120 個品牌：116 通過（96.67%）、136 個 raw payload（8,948,883 bytes）、hash mismatch=0，內容包含 2,303 個 RSS item、21 個 JSON records，以及 64 個 HTML/Browser page-text payload。這不是「今天所有財金資訊」的完整快照；HTML/Browser 沒有統一文章筆數，且各 feed 的窗口與更新頻率不同。完整證據見 [`experiments/news-120/raw-content-acceptance-20260818.json`](experiments/news-120/raw-content-acceptance-20260818.json)。
