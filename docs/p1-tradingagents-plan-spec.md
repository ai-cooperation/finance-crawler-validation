...
- `/v1/ingest/research-report` 與 `/v1/agent/research-reports` 的本機測試覆蓋：私有 R2 物件、D1 索引、`tradingagents_completed` audit、冪等 replay、未選議題、過期時間、外部 evidence、未授權模型執行與未標記 second opinion 均 fail closed；目前 5 個 Worker test files、78 tests 通過，branch coverage 80.09%。
