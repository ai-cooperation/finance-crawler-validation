...
- 子集 batch 只報本批成功數與 target=120，不得將 13/13 寫成整體 100%。
...
- Given 失敗品牌清單，When 指定 13 品牌 batch，Then只發出該 13 品牌的 endpoint requests。
- Given batch 13/13 成功，When 合併 baseline 101/120，Then 必須按被替換的品牌結果計算，不得直接相加重複品牌。
...
- 新增 fallback 後必須由 `ai-cooperation` GitHub runner 重新驗證；本機候選 probe 不算 P2 遠端成功。
