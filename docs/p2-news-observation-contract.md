...
- Raw recovery runs `32116022731`、`32116635535`：只選取四個未通過品牌，最新一輪 3/4 成功，5 個 endpoint payload 全部保存且 hash mismatch=0；Financial Advisor Magazine 與 Financial Express 均為 GitHub 直連 403 後由固定 Cloudflare relay 取得 200。新增 strict RSS validation run `32119549550` 後確認 The TRADE 的 RSS channel 沒有任何 `item/entry`，因此不算有文章內容；ETF Stream 的直連、Crawl4AI 與 relay 都是 Cloudflare JS challenge。嚴格 raw-content 合併 observation 為 118/120，並非宣稱 120/120。
