...
- Strict full run `32119864710`：120 個品牌、141 endpoint attempts、116 成功、136 raw payload、8,948,883 bytes，hash mismatch=0；解析得到 2,303 個 RSS item、21 個 JSON records，另有 64 個 HTML/Browser page-text payload。這是各來源當次可取得內容的觀察，不是「今天所有財金資訊」的完整快照；HTML/Browser 沒有統一文章筆數，來源更新窗口也不同。四個失敗為 International Banker、The TRADE、ETF Stream、ABC Business；先前 bounded recovery 保留為獨立時間點證據，不回填本輪成功率。
