...
`news-sources.yaml` 現在是完整的 120/120 品牌 catalog：100 個財經專業媒體、20 個綜合媒體財經部門。2026-08-13 P2 驗收時共有 162 個巢狀 endpoints；2026-08-14 修路新增 Citywire 的第一方 podcast RSS，並以 Livewire 的第一方 podcast RSS 取代失效網頁，現行 catalog 為 163 條。endpoint 數可以因合規 fallback 成長，但品牌分母固定為 120；歷史 observation 的 endpoint 與成功數不可被新版 catalog 回溯改寫。每個品牌的 canonical domain 唯一；既有 52 或 86 條舊實驗路徑沒有被改名後灌入分母。
