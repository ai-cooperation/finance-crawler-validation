...
2026-08-22 的第一個部署後 full-catalog snapshot（Actions run `32540891073`、BTC job `research_20260822003634_493b163c`）已寫入 `financial-depth`／365 日 CoinGecko 歷史序列、非預測情境表、估值缺口與 lexical source-conflict screening：135 source groups／181 endpoint attempts／149 normalized evidence，coverage `0.9185`，10 個 source groups 失敗，pack 為 `partial`。該舊 snapshot 曾固定產生 `deterministic-evidence-v1` first opinion；它是後續重跑的 frozen source of truth，不代表目前 full-catalog job 的產出策略。時間序列可用（366 點、365 日 observed return `-33.320429%`、年化波動 `43.781446%`、最大回撤 `-53.049398%`），基本面 provider 尚未配置，crypto intrinsic valuation 明確為 `not_applicable`，情境只是 observed range、`not_a_forecast=true`。不含 private raw／token 的 redacted evidence 見 [`experiments/app-a/20260822-btc-professional-full-catalog.json`](../experiments/app-a/20260822-btc-professional-full-catalog.json)。
...
Research Pack 現在會輸出 1／3／7／30／90／365 日 observed return、年化波動與最大回撤。品質修正已在本機以真實公開 provider 驗證：CoinGecko 價格／成交量／tokenomics、Binance Futures funding／open interest／24h volume、Blockchain.com 確認交易數、The Block 公開 ETF-flow JSON 均回傳資料與 response hash；`lexical_stance_v2_calibrated` 也以 frozen calibration set（precision／recall／F1、set hash）通過。對同一組 BTC evidence，`financial_depth.status=professional_ready`，但事件仍標為 `causal_status=unresolved`，所以報告不會把標題直接寫成因果結論。120-brand 全量本機驗證結果為 116／120 成功（96.67%）、166 endpoint attempts、4 個來源 blocked；完整 redacted metadata 見 [`experiments/app-a/20260822-btc-professional-full-local-v8.json`](../experiments/app-a/20260822-btc-professional-full-local-v8.json)。這個 ready 結果尚屬本機 provider bundle 驗證；新的 full-catalog Actions snapshot 尚未在配額閘門解除後重跑，遠端報告仍須以新 snapshot 回讀結果為準。
