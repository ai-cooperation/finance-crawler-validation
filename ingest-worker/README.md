...
5. 使用 `ALERT_WEBHOOK_URL` Worker secret 接上外部失敗通知。`ALERT_WEBHOOK_FORMAT=auto` 會依精確官方 hostname 辨識 Slack Incoming Webhook、Telegram Bot API 與 ntfy；其他 HTTPS URL 使用 version 1 generic JSON。所有外送設 10 秒 timeout、不跟隨 redirect，只在 provider 回覆成功後寫入 D1 receipt。
